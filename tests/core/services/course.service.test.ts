...
describe('`Course` service', () => {
  ...
  courseEntity.findCourses = jest.fn(() =>
    Promise.resolve({
  ...
  test('Create a new Course => should be OK', async () => {
    ...
    expect(await courseService.createCourse(payload)).toEqual({
      title: 'course 1',
      id: 1
    });
  ...
  test('Get Course by ID => should be OK', async () => {
    expect(await courseService.getCourse(1)).toEqual({
      title: 'course 1',
      id: 1
    });
  });
